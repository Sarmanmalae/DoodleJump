    ...
    while running_qwe:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                global running_falling
                running_scores, running_falling, running_qwe, running_start = False, False, False, False
            if event.type == pygame.KEYUP and (event.key == 1073741903 or event.key == 1073741904):
                moving = False
            if event.type == pygame.KEYDOWN and (event.key == 1073741903 or event.key == 1073741904):
                moving = True
